...
var Hlsdump = function (parameters, callback) {
    ...
    Hlsdump.prototype._initialize = function (parameters, callback) {
        ...
        self.dumpChunks = parameters.dumpChunks || false;
    ...
    Hlsdump.prototype._parsePlaylist = function (playlist) {
        ...
        case "streams":
            ...
            break;
        case "media_sequence":
            self._downloadMediaSequence(playlist, function (err, stats) {
                if (err !== null) {
                    ...
                    if (self.duration === null || self.duration > self.downloaded_duration) {
                        ...
                        setTimeout(function (playlist, self) {
                            playlist.get(function (err, newPlaylist, item, stats) {
                                if (err !== null) {
                                    self.emit('error', new Error("Can't getting playlist " + err));
                                } else {
                                    newPlaylist.parent = playlist.id;
    ...
    Hlsdump.prototype._downloadMediaSequence = function (playlist, callback, i) {
        ...
        if (playlist.items[i].type === 'inf') {
            let chunkFilename = playlist.items[i].url;
            chunkFilename = chunkFilename.substring(chunkFilename.lastIndexOf('/') + 1);
            filename = self.temporary_folder  + chunkFilename + ".ts";
            if (self.downloaded_files.indexOf(filename) === -1) {
                playlist.items[i].download(filename, self.dumpChunks, function (err, stats) {
                    self.emit('chunk_stats', stats);
                    if (err !== null) {
                        return callback(err);
                    } else {
    ...
    Hlsdump.prototype.start = function () {
        ...
        self._deleteFile(self.filename, function (err) {
            if (err !== null) {
                ...
                playlist.get(function (err, playlist, item, stats) {
                    if (err !== null) {
                        self.emit('error', new Error("Can't getting playlist " + err));
                    } else {
                        self.playlists.push(playlist);
                        playlist.id = self.playlists.length - 1;
                        self.emit("playlist", playlist, stats);
                    }
                });
            }
        });
